...
class ActionTests extends FlatSpec with Matchers {
  ...
  val data = ActionData(ActionStatus.pending, "test_action", "start.scala", "spark","scala", null, null, Map())
  ...
  "an Action" should "queue it's ActionData int the job queue when executed" in {
    ...
    val action = SequentialAction(data.name, data.src, data.groupId, data.typeId, jobId, queue, client, 1, Map())
